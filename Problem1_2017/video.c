...
void put_in_cache() {
  ...
  for(i = 0; i < C; ++i) {
    ...
    if(cache_score[i][indexes[i][0]] > 0) {
      ++amount_end;
      for(j = 0; j < V; ++j) {
        if(cache_score[i][indexes[i][j]] > 0) {
          if(amount[i] + video_size[indexes[i][j]] <= X) {
            amount[i] += video_size[indexes[i][j]];
            if(i != C - 1)
              for(k = i + 1; k < C; ++k)
                cache_score[k][indexes[i][j]] *= eta;
            results[i][count] = indexes[i][j];
            ++count;
          }
        }
      }
